    ...
    return {
      meta: [
        { title: "Dashboard | KinderKidsSpace" },
        {
          name: "description",
          content:
            "Track your learning progress, view achievements, and continue your educational journey.",
        },
...
function Dashboard() {
  ...
  const abcProgress = Math.min(
    100,
    Math.round(((progress?.lettersLearned || []).length / 26) * 100),
  );
  const numbersProgress = Math.min(
    100,
    Math.round(((progress?.numbersLearned || []).length / 20) * 100),
  );
  const tablesProgress = Math.min(
    100,
    Math.round(((progress?.tablesCompleted || []).length / 10) * 100),
  );
  const playzoneProgress = Math.min(100, Math.round(((progress?.gamesCompleted || 0) / 10) * 100));
  const overallProgress = Math.round(
    (abcProgress + numbersProgress + tablesProgress + playzoneProgress) / 4,
  );
  ...
  const dailyQuests = [
    {
      ...
      cur: Math.min(3, (progress?.lettersLearned || []).length),
      max: 3,
      reward: 10,
    },
    {
      ...
      bg: "bg-[#FEF3C7]",
      title: "Earn 20 stars",
      cur: Math.min(20, progress?.stars || 0),
      max: 20,
      reward: 15,
    },
    {
      ...
      cur: Math.min(1, progress?.attempts || 0),
      max: 1,
      reward: 10,
    },
    {
      icon: imgPlayzone,
      title: "Play 2 games in PlayZone",
      cur: Math.min(2, progress?.gamesCompleted || 0),
      max: 2,
      reward: 15,
    },
  ];
  ...
  if (isLoading) {
    return (
      <div className={`min-h-screen ${mainBg} flex items-center justify-center`}>Loading...</div>
    );
  }

  return (
    <div
      className={`min-h-screen ${mainBg} font-display text-[#374151] flex flex-col md:flex-row relative overflow-hidden`}
    >
      {/* --- DESKTOP SIDEBAR --- */}
      <aside
        className={`hidden md:flex flex-col items-center py-8 w-[110px] ${sidebarBg} border-r ${borderColor} fixed h-full z-40`}
      >
        <div
          className="mb-6 cursor-pointer hover:scale-105 transition-transform mt-2"
          onClick={() => navigate({ to: "/" })}
        >
              ...
              >
                ...
                >
                  {isActive && (
                    <motion.div
                      layoutId="activePill"
                      className="absolute inset-0 bg-[#A7F3D0] rounded-[24px] -z-10"
                    />
                  )}
                  ...
                  <div className="w-12 h-12 mb-0.5">
                    <img
                      src={item.image}
                      alt={item.label}
                      className={`w-full h-full object-contain mix-blend-multiply drop-shadow-sm ${item.scale}`}
                    />
                  </div>
                  <span
                    className={`text-[11px] font-bold ${isActive ? "text-[#059669]" : "text-[#6B7280]"}`}
                  >
                    {item.label}
                  </span>
                </motion.div>
      ...
      {/* --- MOBILE BOTTOM NAV --- */}
      <div
        className={`md:hidden fixed bottom-0 w-full ${sidebarBg} border-t ${borderColor} z-50 pb-safe pt-2 px-4 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] rounded-t-[32px]`}
      >
        <div className="flex justify-between items-center w-full">
            ...
            >
              <div className="w-9 h-9 mb-1">
                <img
                  src={item.image}
                  alt={item.label}
                  className={`w-full h-full object-contain mix-blend-multiply ${item.scale}`}
                />
              </div>
              <span
                className={`text-[10px] font-bold ${activeItem === item.id ? "text-[#059669]" : "text-[#6B7280]"}`}
              >
                {item.label}
              </span>
            </Link>
          ...
          >
            <div
              className={`w-7 h-7 mb-1 flex items-center justify-center ${showMoreMobile ? "text-[#059669]" : "text-[#6B7280]"}`}
            >
              {showMoreMobile ? (
                <X className="w-6 h-6" />
              ) : (
            ...
            <span
              className={`text-[10px] font-bold ${showMoreMobile ? "text-[#059669]" : "text-[#6B7280]"}`}
            >
              More
            </span>
          </button>
        ...
        {showMoreMobile && (
          ...
          >
            {SIDEBAR_ITEMS.slice(4).map((item) => (
              ...
              >
                <div
                  className={`w-14 h-14 rounded-[20px] ${item.bgColor} flex items-center justify-center mb-2 overflow-hidden`}
                >
                  <img
                    src={item.image}
                    alt={item.label}
                    className={`w-12 h-12 object-contain mix-blend-multiply ${item.scale}`}
                  />
                </div>
                <span className="text-[10px] font-bold text-center text-[#6B7280]">
                  {item.label}
                </span>
              </Link>
            ))}
      ...
      <main className="flex-1 md:ml-[110px] w-full min-h-screen pb-24 md:pb-8 relative">
        ...
        <header className="h-[80px] sticky top-0 z-30 flex items-center justify-between px-6 lg:px-12 backdrop-blur-md bg-[#F5EFE4]/80">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link
              to="/profile"
              className="flex items-center gap-2 bg-white pl-2 pr-4 py-1.5 rounded-full shadow-sm border border-[#EADCC6] cursor-pointer hover:bg-gray-50 transition-colors"
            >
        ...
        <div className="px-4 md:px-6 lg:px-12 max-w-[1300px] mx-auto flex flex-col gap-8 pb-10">
          {/* WELCOME HERO CARD */}
          <div
            className={`${cardBg} rounded-[32px] border ${borderColor} p-6 md:p-8 flex flex-col lg:flex-row gap-6 lg:gap-12 relative overflow-hidden shadow-[0_20px_40px_-15px_rgba(0,0,0,0.03)]`}
          >
            {/* Left Content */}
            <div className="flex-1 z-10 flex flex-col justify-center">
              ...
              <h1 className="text-4xl md:text-5xl font-extrabold text-[#374151] mb-2 flex items-center gap-2">
                {username || "Champion"}!{" "}
                <span className="animate-wiggle origin-bottom-right inline-block">👋</span>
              </h1>
              ...
              {[
                {
                  ...
                  color: "text-[#EF4444]",
                  label: "Day Streak",
                  val: (progress?.streakDays || 0).toString(),
                  bg: "bg-[#FEE2E2]",
                },
                {
                  icon: Star,
                ...
                <div
                  key={i}
                  className="bg-white rounded-[24px] p-3 md:p-4 flex items-center gap-3 min-w-[140px] shadow-sm border border-[#EADCC6]"
                >
                  <div
                    className={`w-10 h-10 md:w-12 md:h-12 rounded-full ${stat.bg} flex items-center justify-center shrink-0`}
                  >
                    <stat.icon
                      className={`w-5 h-5 md:w-6 md:h-6 ${stat.color} ${stat.icon === Star ? "fill-current" : ""}`}
                    />
                  </div>
                  <div className="flex flex-col">
                    <span className="font-extrabold text-[#374151] text-base md:text-lg leading-tight">
                      {stat.val}
                    </span>
                    <span className="font-bold text-[#6B7280] text-[10px] md:text-xs leading-tight">
                      {stat.label}
                    </span>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 lg:gap-8">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div
                className={`${cardBg} rounded-[32px] border ${borderColor} p-6 flex items-center justify-between shadow-sm relative overflow-hidden h-full min-h-[220px]`}
              >
                <div className="z-10 flex flex-col h-full justify-between max-w-[60%]">
                  <div>
                    <h4 className="font-extrabold text-[#374151] text-xl md:text-2xl mb-1">
                      {continueActivity.title}
                    </h4>
                  ...
                  <div className="mt-4 mb-6">
                    <div className="flex items-center gap-3 mb-1">
                      <div className="h-2 bg-[#E5E7EB] rounded-full flex-1 max-w-[120px] overflow-hidden">
                        <div
                          className="h-full bg-[#65A30D] rounded-full"
                          style={{ width: `${continueActivity.percent}%` }}
                        />
                      </div>
                      <span className="font-bold text-sm text-[#374151]">
                        {continueActivity.percent}%
                      </span>
                    </div>
                  </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div
                className={`${cardBg} rounded-[32px] border ${borderColor} p-6 flex flex-col justify-between shadow-sm h-full`}
              >
                <div className="flex flex-col gap-3">
                  {dailyQuests.map((quest, i) => (
                    <div
                      key={i}
                      className={`flex items-center justify-between pb-3 border-b border-[#EADCC6]/50 last:border-0 last:pb-0 ${quest.cur >= quest.max ? "opacity-50 grayscale" : ""}`}
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-lg bg-white/50 flex items-center justify-center overflow-hidden`}
                        >
                          {typeof quest.icon === "string" ? (
                            ...
                            <quest.icon
                              className={`w-4 h-4 ${quest.color} ${quest.icon === Star ? "fill-current" : ""}`}
                            />
                          )}
                        </div>
            ...
            <div className="xl:col-span-2 flex flex-col gap-4">
              ...
              <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar min-h-[140px]">
                {earnedBadges.length === 0 ? (
                  <div
                    className={`${cardBg} rounded-[24px] border ${borderColor} p-6 w-full flex flex-col items-center justify-center text-center shadow-sm`}
                  >
                  ...
                  earnedBadges.map((ach, i) => (
                    <div
                      key={i}
                      className={`${cardBg} rounded-[24px] border ${borderColor} p-4 min-w-[150px] flex flex-col items-center text-center shadow-sm hover:shadow-md transition-shadow`}
                    >
                      <div
                        className={`w-16 h-16 rounded-[16px] bg-[#ECFCCB] flex items-center justify-center mb-3 relative overflow-hidden`}
                      >
              ...
              <Link to="/rewards">
                ...
                >
                  <div>
                    ...
                    <p className="text-xs font-semibold text-[#6B7280] max-w-[120px] mb-4">
                      Open your chest and see your rewards!
                    </p>
