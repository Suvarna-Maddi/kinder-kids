...
const BADGES: Array<{ slug: string; label: string; when: (p: ReturnType<typeof useProgress>) => boolean; emoji: string }> = [
  ...
  { slug: "alpha-half",   label: "Halfway ABC",      when: (p) => p.lettersLearned.length >= 13, emoji: "🔤" },
  { slug: "alpha-full",   label: "Alphabet Master",  when: (p) => p.lettersLearned.length >= 26, emoji: "📖" },
  { slug: "number-20",    label: "Number Explorer",  when: (p) => p.numbersLearned.length >= 20, emoji: "🔢" },
  { slug: "tables-5",     label: "Times Champion",   when: (p) => p.tablesCompleted.length >= 5,  emoji: "✖️" },
...
const Rewards = () => {
  ...
  const stats = [
    { label: "Stars",         value: p.stars,                  icon: Star,       color: "text-kid-yellow", bg: "bg-kid-yellow/10" },
    { label: "Coins",         value: p.coins,                  icon: Coins,      color: "text-kid-orange", bg: "bg-kid-orange/10" },
    { label: "Day Streak",    value: p.streakDays,             icon: Flame,      color: "text-kid-red",    bg: "bg-kid-red/10" },
    { label: "Games Played",  value: p.gamesCompleted,         icon: Trophy,     color: "text-kid-purple", bg: "bg-kid-purple/10" },
    { label: "Letters Learned", value: p.lettersLearned.length, icon: BookOpen,  color: "text-kid-green",  bg: "bg-kid-green/10" },
    { label: "Numbers Learned", value: p.numbersLearned.length, icon: Hash,      color: "text-kid-blue",   bg: "bg-kid-blue/10" },
    { label: "Tables Done",     value: p.tablesCompleted.length, icon: Grid3X3, color: "text-kid-pink",   bg: "bg-kid-pink/10" },
    { label: "Accuracy",        value: `${accuracy}%`,          icon: Sparkles, color: "text-kid-teal",   bg: "bg-kid-teal/10" },
  ];
    ...
    <div className="p-4 md:p-8">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="text-3xl md:text-5xl font-display font-bold text-foreground">Your Rewards 🏆</h1>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-5 mb-10">
          {stats.map((s, i) => {
            ...
            return (
              ...
              >
                <div className={`${s.bg} w-12 h-12 mx-auto rounded-2xl flex items-center justify-center mb-2`}>
