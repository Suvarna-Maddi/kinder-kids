...
import { Sparkles, PlayCircle, CheckCircle, XCircle, ChevronLeft, ChevronRight, Volume2, Lock, Unlock, Thermometer } from "lucide-react";
...
const SolarSystem = () => {
  ...
  const handlePlanetClick = (planet: PlanetData) => {
    ...
    setTimeout(() => {
      document.getElementById("planet-details")?.scrollIntoView({ behavior: "smooth", block: "center" });
    }, 500);
  };
    ...
    >
      ...
      >
        ...
        <div 
          ...
          style={{ backgroundImage: "url('https://images.unsplash.com/photo-1506703719100-a0f3a48c0f86?q=80&w=2560&auto=format&fit=crop')" }}
        />
        ...
        <motion.div 
          ...
          style={{ backgroundImage: 'radial-gradient(circle, #ffffff 1px, transparent 1px)', backgroundSize: '70px 70px', backgroundPosition: '0 0' }} 
        />
        <motion.div 
          className="absolute inset-0" 
          animate={{ opacity: [0.1, 0.5, 0.1] }} 
          transition={{ duration: 3, repeat: Infinity, ease: "easeInOut", delay: 1 }} 
          style={{ backgroundImage: 'radial-gradient(circle, #ffffff 1.5px, transparent 1px)', backgroundSize: '130px 130px', backgroundPosition: '65px 65px' }} 
        />
        <motion.div 
          className="absolute inset-0" 
          animate={{ opacity: [0.05, 0.3, 0.05] }} 
          transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 2.5 }} 
          style={{ backgroundImage: 'radial-gradient(circle, #a5b4fc 2px, transparent 2px)', backgroundSize: '220px 220px', backgroundPosition: '110px 110px' }} 
        />
      ...
      <div className="relative z-10 w-full pt-12 pb-24">
        ...
        <div className="w-full mb-16 relative overflow-x-auto overflow-y-visible pb-16 pt-8 snap-x scrollbar-thin scrollbar-thumb-indigo-500/30">
          <div className="flex items-center min-w-max px-8 md:px-24 h-64 gap-8 md:gap-16 relative">
            ...
            <div className="relative shrink-0 w-64 h-64 md:w-80 md:h-80 -ml-16 md:-ml-24 z-0 pointer-events-none flex items-center justify-center">
              <div className="absolute inset-0 bg-orange-500/30 blur-[100px] rounded-full mix-blend-screen" />
              <div className="absolute inset-0 bg-yellow-400/20 blur-[50px] rounded-full mix-blend-screen animate-pulse" />
              <img 
                src={IMG_MAP["Sun"]} 
                alt="The Sun" 
                className="w-full h-full object-contain drop-shadow-[0_0_80px_rgba(255,150,0,0.8)]"
                loading="lazy"
                decoding="async"
              />
            </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="w-[180px] h-[180px] flex items-center justify-center relative overflow-visible">
                    ...
                    <AnimatePresence>
                      {isSelected && (
                        ...
                        >
                          <div className={`absolute inset-0 bg-gradient-to-tr ${planet.gradient} opacity-40 blur-3xl rounded-full mix-blend-screen animate-pulse`} />
                          
                          {/* Animated Orbital Ring */}
                          <svg className="absolute w-[220px] h-[220px] animate-[spin_10s_linear_infinite] overflow-visible" viewBox="0 0 100 100">
                            <circle cx="50" cy="50" r="48" fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="0.5" strokeDasharray="4 4" />
                            <circle cx="50" cy="2" r="2" fill="#fff" className="shadow-[0_0_10px_#fff]" />
                            <circle cx="98" cy="50" r="1.5" fill="#fff" className="shadow-[0_0_10px_#fff]" />
                          </svg>
                        </motion.div>
                      )}
                    </AnimatePresence>
                    ...
                    >
                      <img
                        ...
                        style={{ 
                          filter: isSelected ? 'drop-shadow(0 0 25px rgba(255,255,255,0.4)) drop-shadow(0 0 50px rgba(255,255,255,0.2))' : 'drop-shadow(0 0 10px rgba(0,0,0,0.8))'
                        }}
                        draggable={false}
                        loading="lazy"
                        decoding="async"
                  ...
                  <div className="text-center mt-2 relative z-20">
                    <h3 className={`font-display text-2xl font-bold tracking-wide ${planet.color} drop-shadow-[0_0_8px_rgba(255,255,255,0.3)] transition-colors`}>
                      {planet.name}
                    </h3>
                  </div>
                </motion.button>
          ...
          <AnimatePresence mode="wait">
            {selectedPlanet && (
              ...
              >
                ...
                <div className={`absolute top-0 right-0 w-[800px] h-[800px] bg-gradient-to-bl ${selectedPlanet.gradient} opacity-[0.15] blur-[100px] rounded-full pointer-events-none -translate-y-1/2 translate-x-1/4 mix-blend-screen`} />
                ...
                <div className="flex flex-col lg:flex-row gap-12 items-center lg:items-start relative z-10">
                  ...
                  <div className="w-full lg:w-7/12 flex flex-col">
                    <div className="flex items-center gap-5 mb-2">
                      <h2 className={`text-5xl md:text-7xl font-display font-bold tracking-tight ${selectedPlanet.color} drop-shadow-[0_0_20px_currentColor]`}>
                        {selectedPlanet.name}
                      </h2>
                      <button onClick={playName} className="p-4 bg-white/5 hover:bg-white/15 text-white rounded-full transition-all border border-white/10 hover:border-white/30 shadow-lg">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8 text-sm md:text-base">
                      {[
                        ...
                        { label: "Avg Temp", val: selectedPlanet.avgTemp || "N/A", icon: Thermometer }
                      ].map((stat, idx) => (
                        <div key={idx} className="bg-[#050510]/50 p-4 rounded-2xl border border-white/5 flex flex-col">
                          ...
                          <span className="font-bold text-white text-lg tracking-wide">{stat.val}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="bg-[#050510]/80 rounded-3xl p-8 border border-indigo-500/20 shadow-inner">
                      <h3 className="text-2xl font-display font-bold text-indigo-300 mb-2">Knowledge Check</h3>
                      <p className="text-xl text-slate-200 mb-6">{selectedPlanet.quiz.question}</p>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {selectedPlanet.quiz.options.map((opt) => {
                          const isCorrect = quizFeedback === "correct" && opt === selectedPlanet.quiz.correct;
                          const isWrong = quizFeedback === "wrong" && opt === quizPicked;
                          let btnClass = "bg-white/5 hover:bg-white/10 text-slate-200 border-white/10";
                          if (isCorrect) btnClass = "bg-emerald-500/20 text-emerald-300 border-emerald-500/50 shadow-[0_0_20px_rgba(16,185,129,0.2)]";
                          else if (isWrong) btnClass = "bg-rose-500/20 text-rose-300 border-rose-500/50";
        ...
        <div className="mt-32 max-w-5xl mx-auto text-center px-4 relative z-10">
          <div className="flex items-center justify-center gap-4 mb-10">
            ...
            {allViewed ? <Unlock className="w-10 h-10 text-emerald-400" /> : <Lock className="w-10 h-10 text-slate-500" />}
          </div>
          
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {UNLOCKABLES.map((item) => {
              const isUnlocked = allViewed;
              return (
                <div 
                  key={item.id}
                  className={`p-6 rounded-3xl border backdrop-blur-md ${isUnlocked ? 'bg-indigo-900/30 border-indigo-400/30 shadow-[0_0_30px_rgba(99,102,241,0.2)]' : 'bg-[#050510]/60 border-white/5'} transition-all duration-700`}
                >
                  <div className={`text-5xl mb-4 ${isUnlocked ? 'opacity-100 scale-110 transition-transform duration-500 ease-out' : 'opacity-20 grayscale'}`}>
                    {item.emoji}
                  </div>
                  <div className={`font-bold text-lg tracking-wide ${isUnlocked ? 'text-indigo-200' : 'text-slate-600'}`}>
          ...
          {!allViewed && (
            <p className="mt-8 text-slate-400 font-medium text-lg">
              Explore all {SOLAR_SYSTEM.length - 1} planets to unlock these space secrets!
            </p>
          )}
          {allViewed && (
            <motion.p 
            ...
            >
              Congratulations! You've unlocked the mysteries of the universe! 🚀
            </motion.p>
          )}
        </div>
